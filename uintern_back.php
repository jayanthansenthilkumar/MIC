...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    switch($action) {
        case 'fetch':
            ...
            $sql = "SELECT uid, iname, ayear, cert FROM sintern WHERE ayear IS NULL";
        ...
        case 'update':
            ...
            $sql = "UPDATE sintern SET ayear = ? WHERE uid = ?";
        ...
        case 'delete':
            ...
            $sql = "SELECT cert FROM sintern WHERE uid = ?";
            $stmt = $conn->prepare($sql);
            ...
            $sql = "DELETE FROM sintern WHERE uid = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $recordId);
