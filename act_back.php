...
function getDepartmentStats($conn, $year = '', $batch = '') {
    $yearCondition = '';
    $batchCondition = '';

    if (!empty($year)) {
        $yearCondition = " AND i.ayear = '$year'"; // for internships
    }

    if (!empty($batch)) {
        $batchCondition = " AND s.ayear = '$batch'";
    }
    ...
    $queries = [
        'internships' => [
            ...
            WHERE 1=1" . $yearCondition .$batchCondition . "
            GROUP BY s.dept"
        ],
        'certifications' => [
            "SELECT 
            ...
            WHERE 1=1" . str_replace('i.ayear', 'c.ayear', $yearCondition) . $batchCondition . "
            GROUP BY s.dept"
        ],
        'projects' => [
            "SELECT 
            ...
            WHERE 1=1" . str_replace('i.ayear', 'p.ayear', $yearCondition) . $batchCondition . "
            GROUP BY s.dept"
        ],
        'languages' => [
            "SELECT 
            ...
            WHERE 1=1" . str_replace('i.ayear', 'l.ayear', $yearCondition) . $batchCondition . "
            GROUP BY s.dept"
        ],
        'cocurricular' => [
            "SELECT 
        ...
        'extracurricular' => [
            ...
            WHERE 1=1" . str_replace('i.ayear', 'e.ayear', $yearCondition) . $batchCondition . "
            GROUP BY s.dept"
        ],
        'placements' => [
            "SELECT 
    ...
    foreach ($queries as $key => $queryData) {
        $stmt = $conn->query($queryData[0]); // Directly execute the query
        $data = [];
...
function getFacultyStats($conn, $year = '', $batch = '') {
...
if (isset($_GET['type'])) {
    ...
    switch ($type) {
        case 'department':
            $response = getDepartmentStats($conn, $year, $batch);
            break;
        case 'faculty':
            $response = getFacultyStats($conn, $year, $batch);
            break;
        default:
