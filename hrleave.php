        ...
        <div class="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="row" style="justify-content: center;">
                    <div class="leaveAvailability"
                        style="background: linear-gradient(to top, lightgrey 0%, lightgrey 1%, #e0e0e0 26%, #efefef 48%, #d9d9d9 75%, #bcbcbc 100%);">
                        <div class="leaveAvailabilityHeader">
                            <span>Leaves</span>
                            <span>Availability</span>
                        </div>
                        <div class="leaveAvailabilityValues">
                            <?php
                            $leaveTypes = [
                                'cl' => 'Casual Leave',
                                'col' => 'Compensation Leave',
                                'odb' => 'OnDuty Basic',
                                'odr' => 'On Duty Research',
                                'odp' => 'On Duty Professional',
                                'odo' => 'On Duty Outreach',
                                'vl' => 'Vacation Leave',
                                'ml' => 'Medical Leave',
                                'mal' => 'Marriage Leave',
                                'mtl' => 'Maternity Leave',
                                ...
                                'spl' => 'Special Leave',
                                'pm' => 'Permission',
                                'tenpm' => '10min Permission'
                            ];
                            foreach ($leaveTypes as $key => $title) {
                                echo "<span class='leave-value' data-leave-type='$key' title='$title'>" . strtoupper($key) . ": 0</span>";
                            }
                            ?>
                        </div>
                    </div>
    ...
    <script>
        ...
        function loadLeaveDetails() {
            $.ajax({
                ...
                method: 'GET',
                data: { action: "get_leave" },
                dataType: 'json',
                success: function (response) {
                    if (response && response.length > 0) {
                        var data = response[0];
                        $('.leave-value').each(function () {
                            var leaveType = $(this).data('leave-type');
                            var leaveValue = data[leaveType] || 0;
                            $(this).text(leaveType.toUpperCase() + ': ' + leaveValue);

                            // Update background color
                            var background = leaveValue == 0 ?
