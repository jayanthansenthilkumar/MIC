...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    switch($action) {
        case 'fetch':
            ...
            $sql = "SELECT uid, event, ayear, cert FROM st_extra WHERE ayear IS NULL";
        ...
        case 'update':
            ...
            $sql = "UPDATE st_extra SET ayear = ? WHERE uid = ?";
        ...
        case 'delete':
            ...
            $sql = "SELECT cert FROM st_extra WHERE uid = ?";
            $stmt = $conn->prepare($sql);
            ...
            $sql = "DELETE FROM st_extra WHERE uid = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $recordId);
